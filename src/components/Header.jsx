...
export default function Header() {
    ...
    const userOptions = [
        ...
        {
            name: "Documentation",
            to: "https://github.com/bkvats/RBAC",
        ...
        <header className="sticky top-0 bg-[#EFEFEF] dark:bg-gray-950 min-w-full">
            <div className="container mx-auto flex px-4 min-h-16 items-center justify-between">
                <div className="flex flex-col items-center gap-1">
                    <Link className="title-font font-medium text-xl text-gray-900 dark:text-white">
                        RBAC
                    </Link>
                    <SwitchTheme defaultToggleValue={localStorage.getItem("theme") ? true : undefined}/>
                </div>
                <nav className="ml-auto hidden lg:block text-base">
                    {
                        navLiks.map(i => (
                            <Link key={i.to} to={i.to} className="hover:underline hover:text-gray-900 mx-8 dark:text-gray-500 dark:hover:text-gray-600">{i.title}</Link>
                        ))
                    }
                </nav>
                {isLoggedIn ? <div>
                        ...
                        <ul className="bg-white dark:bg-gray-700 w-full py-1 px-2 text-lg">
                            {
                                userOptions.map((i) => (
                                    <li key={i.name} className="hover:underline my-2 dark:text-white">
                                        <Link to={i.to} target={i.target} onClick={() => {setUserOptionsVisible(false)}} className="flex items-center space-x-2">
                                            {i.icon}
                                            <span>{i.name}</span>
                                        </Link>
                                    </li>
                ...
                </div> : <Link to={"/login"} className="flex items-center border-2 bg-gray-100 py-1 px-3 focus:outline-none hover:bg-gray-200 rounded text-base">Try Now
